  ...
  switch (action.type) {
    case "inisialState":
      return {
        ...
        total_row: action.payload.total_row,
      };
      break;
    case "updatepage":
      return {
...
const AllStudents = (props) => {
  ...
  useEffect(() => {
    ...
    if (props.list.length > 0) {
      ...
      let counterList = [...Array(counter)].map((i, k) => {
        return k + 1;
      }); 
      if(state.currentPage > 1)
      {
        counterList = [...state.pages]
      } 
      const updateState = {
        pageCount: Math.ceil(props.count / 10),
        pages: counterList,
        total_row: props.count,
      };
          ...
          <div className="card shadow">
            ...
            <div className="card-footer py-4">
              <span className="float-left text-muted">
                {" "}
                Total records found {state.total_row}
              </span>
              <nav aria-label="...">
